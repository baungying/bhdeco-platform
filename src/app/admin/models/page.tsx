...
interface Model { id?:number;title:string;category:string;description:string;image_url:string;file_url:string;credits_cost:number;active:boolean; }
const EMPTY_MODEL: Model = { title:"",category:"SketchUp Models",description:"",image_url:"",file_url:"",credits_cost:80,active:true };
...
export default function ModelsPage() {
  ...
  const F = ({label,k,type="text",opts}:{label:string;k:keyof Model;type?:string;opts?:string[]}) => (
    <div>
      <label style={{display:"block",fontFamily:"monospace",fontSize:".55rem",color:"#7A8FA8",letterSpacing:".1em",textTransform:"uppercase",marginBottom:".35rem"}}>{label}</label>
      {opts
        ? <select value={String(editing![k])} onChange={e=>setEditing(p=>({...p!,[k]:e.target.value}))} style={inp} onFocus={fi as never} onBlur={bl as never}>
            {opts.map(o=><option key={o} value={o}>{o}</option>)}
          </select>
        : type==="textarea"
          ? <textarea value={String(editing![k]||"")} onChange={e=>setEditing(p=>({...p!,[k]:e.target.value}))} rows={3} style={{...inp,resize:"vertical"} as React.CSSProperties} onFocus={fi as never} onBlur={bl as never}/>
          : <input type={type} value={type==="number"?Number(editing![k]):String(editing![k]||"")} onChange={e=>setEditing(p=>({...p!,[k]:type==="number"?Number(e.target.value):e.target.value}))} style={inp} onFocus={fi} onBlur={bl}/>
      }
    </div>
  );
      ...
      <div style={{maxWidth:"1100px",margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"2rem",flexWrap:"wrap",gap:"1rem"}}>
          <div>
            ...
            <h1 style={{fontSize:"clamp(1.8rem,3vw,2.4rem)",fontWeight:"300",fontFamily:"'Cormorant Garant',serif"}}>3D <span style={{fontStyle:"italic",color:G}}>Models</span></h1>
          </div>
          <button onClick={()=>setEditing({...EMPTY_MODEL})}
        ...
        {editing&&(
          <div style={{background:"rgba(212,167,61,.04)",border:`1px solid rgba(212,167,61,.2)`,borderRadius:"12px",padding:"1.75rem",marginBottom:"2rem"}}>
            <h3 style={{fontFamily:"monospace",fontSize:".65rem",color:G,letterSpacing:".14em",marginBottom:"1.25rem"}}>{editing.id?"EDIT MODEL":"NEW MODEL"}</h3>
            <form onSubmit={save} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
              <div style={{gridColumn:"1 / -1"}}><F label="Title" k="title"/></div>
              <F label="Category" k="category" opts={CATS}/>
              <F label="Credits Cost" k="credits_cost" type="number"/>
              <div style={{gridColumn:"1 / -1"}}><F label="Description" k="description" type="textarea"/></div>
              <F label="Image URL" k="image_url"/>
              <F label="File Download URL" k="file_url"/>
              <div style={{gridColumn:"1 / -1",display:"flex",alignItems:"center",gap:".75rem"}}>
                <input type="checkbox" id="model_active" checked={editing.active} onChange={e=>setEditing(p=>({...p!,active:e.target.checked}))} style={{width:"16px",height:"16px",accentColor:G}}/>
                <label htmlFor="model_active" style={{fontSize:".82rem",color:"#EDE8DC"}}>Active (visible on website)</label>
              </div>
              ...
              <div style={{gridColumn:"1 / -1",display:"flex",gap:".75rem"}}>
                <button type="submit" disabled={saving} style={{padding:".7rem 1.6rem",borderRadius:"8px",border:"none",background:`linear-gradient(135deg,${G},#9A7020)`,color:"#010408",fontWeight:"700",fontSize:".75rem",letterSpacing:".08em",textTransform:"uppercase",cursor:saving?"wait":"pointer"}}>
                  {saving?"Saving…":"Save Model"}
                </button>
                <button type="button" onClick={()=>setEditing(null)} style={{padding:".7rem 1.2rem",borderRadius:"8px",border:"1px solid rgba(255,255,255,.1)",background:"transparent",color:"#7A8FA8",fontSize:".75rem",cursor:"pointer"}}>
                  Cancel
                </button>
              </div>
            </form>
        ...
        {/* Table */}
        {loading?<p style={{color:"#2E4060"}}>Loading…</p>:rows.length===0?<p style={{color:"#2E4060",fontFamily:"monospace",fontSize:".65rem",letterSpacing:".1em"}}>NO MODELS YET — ADD YOUR FIRST ONE</p>:(
            ...
            {rows.map((r,i)=>(
              <div key={r.id} style={{display:"grid",gridTemplateColumns:"1fr 120px 80px 80px 100px",gap:"1rem",padding:".9rem 1.25rem",borderBottom:i<rows.length-1?"1px solid rgba(255,255,255,.04)":"none",alignItems:"center",transition:"background .15s"}}
                ...
                <div>
                  ...
                  <p style={{fontSize:".7rem",color:"#7A8FA8"}}>{r.category}</p>
                </div>
                <span style={{fontFamily:"monospace",fontSize:".72rem",color:G}}>⬡ {r.credits_cost}</span>
                <span style={{fontFamily:"monospace",fontSize:".62rem",padding:".2rem .5rem",borderRadius:"100px",background:r.active?"rgba(16,185,129,.1)":"rgba(255,255,255,.05)",color:r.active?"#10B981":"#2E4060"}}>{r.active?"ON":"OFF"}</span>
