...
const CATS = ["C Channel Frame","Container House","Hardware Accessories","Boards & Panels","Floor Material","Wall & Ceiling","Mobihome"];

interface Product { id?:number;name:string;category:string;description:string;image_url:string;price:string;active:boolean; }
const EMPTY: Product = { name:"",category:"C Channel Frame",description:"",image_url:"",price:"",active:true };
      ...
      <div style={{maxWidth:"1100px",margin:"0 auto"}}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:"2rem",flexWrap:"wrap",gap:"1rem"}}>
          <div>
            ...
            <h1 style={{fontSize:"clamp(1.8rem,3vw,2.4rem)",fontWeight:"300",fontFamily:"'Cormorant Garant',serif"}}>Product <span style={{fontStyle:"italic",color:G}}>Catalogue</span></h1>
          </div>
          <button onClick={()=>setEditing({...EMPTY})} style={{padding:".65rem 1.4rem",borderRadius:"8px",border:`1px solid ${G}`,background:"transparent",color:G,fontSize:".75rem",letterSpacing:".08em",textTransform:"uppercase",cursor:"pointer"}}>+ Add Product</button>
        </div>
        ...
        {editing&&(
          <div style={{background:"rgba(212,167,61,.04)",border:`1px solid rgba(212,167,61,.2)`,borderRadius:"12px",padding:"1.75rem",marginBottom:"2rem"}}>
            <h3 style={{fontFamily:"monospace",fontSize:".65rem",color:G,letterSpacing:".14em",marginBottom:"1.25rem"}}>{editing.id?"EDIT PRODUCT":"NEW PRODUCT"}</h3>
            <form onSubmit={save} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"1rem"}}>
              {([
                {k:"name",        label:"Product Name",  col:"1/-1"},
                {k:"category",    label:"Category",      col:"auto", opts:CATS},
                {k:"price",       label:"Price (optional)", col:"auto"},
                {k:"description", label:"Description",   col:"1/-1", ta:true},
                {k:"image_url",   label:"Image URL",     col:"1/-1"},
              ] as {k:keyof Product;label:string;col?:string;opts?:string[];ta?:boolean}[]).map(f=>(
              ...
              <div style={{gridColumn:"1/-1",display:"flex",alignItems:"center",gap:".75rem"}}>
                <input type="checkbox" id="prod_active" checked={editing.active} onChange={e=>setEditing(p=>({...p!,active:e.target.checked}))} style={{width:"16px",height:"16px",accentColor:G}}/>
                <label htmlFor="prod_active" style={{fontSize:".82rem",color:"#EDE8DC"}}>Active (visible on website)</label>
              </div>
              ...
              <div style={{gridColumn:"1/-1",display:"flex",gap:".75rem"}}>
                <button type="submit" disabled={saving} style={{padding:".7rem 1.6rem",borderRadius:"8px",border:"none",background:`linear-gradient(135deg,${G},#9A7020)`,color:"#010408",fontWeight:"700",fontSize:".75rem",letterSpacing:".08em",textTransform:"uppercase",cursor:saving?"wait":"pointer"}}>{saving?"Saving…":"Save Product"}</button>
        ...
        {loading?<p style={{color:"#2E4060"}}>Loading…</p>:rows.length===0?<p style={{color:"#2E4060",fontFamily:"monospace",fontSize:".65rem",letterSpacing:".1em"}}>NO PRODUCTS YET</p>:(
          <div style={{border:"1px solid rgba(255,255,255,.07)",borderRadius:"12px",overflow:"hidden"}}>
            {rows.map((r,i)=>(
              <div key={r.id} style={{display:"grid",gridTemplateColumns:"1fr 140px 80px 80px 100px",gap:"1rem",padding:".9rem 1.25rem",borderBottom:i<rows.length-1?"1px solid rgba(255,255,255,.04)":"none",alignItems:"center",transition:"background .15s"}}
                onMouseEnter={e=>(e.currentTarget.style.background="rgba(255,255,255,.02)")}
                onMouseLeave={e=>(e.currentTarget.style.background="transparent")}>
                <div>
                  <p style={{fontSize:".85rem",color:"#EDE8DC",fontWeight:"500"}}>{r.name}</p>
                  <p style={{fontSize:".7rem",color:"#7A8FA8"}}>{r.category}{r.price?` · ${r.price}`:""}</p>
                </div>
                <span style={{fontFamily:"monospace",fontSize:".62rem",color:"#7A8FA8"}}>{r.category}</span>
