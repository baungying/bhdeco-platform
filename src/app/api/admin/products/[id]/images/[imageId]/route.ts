...
export async function PUT(req: NextRequest, context: { params: Promise<{ id:string; imageId:string }> }) {
  if (!guard(req)) return Response.json({ error:"Unauthorized" },{ status:401 });
  const { imageId } = await context.params;
  const body = await req.json();
  const update = Object.fromEntries(
    Object.entries(body).filter(([k])=>["image_type","caption","sort_order"].includes(k))
  );
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ id:string; imageId:string }> }) {
  if (!guard(req)) return Response.json({ error:"Unauthorized" },{ status:401 });
  const { imageId } = await context.params;
